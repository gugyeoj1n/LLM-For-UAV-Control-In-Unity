...
public class SatelliteVisionAnalyzer : MonoBehaviour
{
    ...
    async void Start()
    {
        Debug.Log("SatelliteVisionAnalyzer 초기화 중...");
        
        // 카메라가 없으면 자식 오브젝트에서 찾기
        if (SatelliteCamera == null)
        {
            SatelliteCamera = GetComponentInChildren<Camera>();
            if (SatelliteCamera == null)
            {
        ...
        SatelliteCamera.targetTexture = renderTexture;
    ...
    void ProcessAnalysisResult(string jsonResult)
    {
        ...
        UnityMainThreadDispatcher.Instance().Enqueue(() => {
            try {
                ...
                // NotifySatelliteController(result);
            }
            catch (System.Exception e) {
    ...
    IEnumerator TestCaptureRoutine()
    {
        ...
        System.IO.File.WriteAllBytes(Application.persistentDataPath + "/Satellite_capture_test.png", pngBytes);
        
        Debug.Log($"테스트 이미지 저장 완료: {Application.persistentDataPath}/Satellite_capture_test.png");
